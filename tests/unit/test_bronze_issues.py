...
class TestExtractIssues:
    ...
    def test_extract_issues_success(self):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=mock_repos), patch('coops.bronze.issues.save_json_data', return_value="file.json") as mock_save:
    ...
    def test_extract_issues_separates_prs_from_issues(self):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=mock_repos), patch('coops.bronze.issues.save_json_data', side_effect=capture_save):
            extract_issues(mock_client, mock_config)

            # Verifica que salvou arquivos separados
            assert any('issues_' in k for k in saved_data)
            assert any('prs_' in k for k in saved_data)
    ...
    def test_extract_issues_filters_event_fields(self):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=mock_repos), patch('coops.bronze.issues.save_json_data', side_effect=capture_save):
            ...
            events_key = next((k for k in saved_data if 'issue_events_' in k), None)
    ...
    def test_extract_issues_retires_stale_aggregates(self, tmp_path, monkeypatch):
        ...
        with patch('coops.bronze.issues.load_json_data',
                   return_value=[{"name": "repo1", "full_name": "test-org/repo1"}]), patch('coops.bronze.issues.save_json_data', return_value="file.json") as mock_save:
            extract_issues(mock_client, mock_config)
    ...
    def test_extract_issues_removal_spares_silver(self, tmp_path, monkeypatch):
        ...
        with patch('coops.bronze.issues.load_json_data',
                   return_value=[{"name": "repo1", "full_name": "test-org/repo1"}]), patch('coops.bronze.issues.save_json_data', return_value="file.json"):
            extract_issues(mock_client, mock_config)
    ...
    def test_extract_issues_uses_cache_flag(self):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=mock_repos), patch('coops.bronze.issues.save_json_data', return_value="file.json"):
    ...
    def test_extract_issues_adds_repo_name_to_issues(self):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=mock_repos), patch('coops.bronze.issues.save_json_data', side_effect=capture_save):
            ...
            issues_key = next((k for k in saved_data if 'issues_test-repo' in k), None)
    ...
    def test_extract_issues_adds_repo_name_to_prs(self):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=mock_repos), patch('coops.bronze.issues.save_json_data', side_effect=capture_save):
            ...
            prs_key = next((k for k in saved_data if 'prs_test-repo' in k), None)
...
class TestExtractIssuesCaps:
    ...
    REPOS: ClassVar = [{"name": "repo1", "full_name": "test-org/repo1"}]
    MIXED: ClassVar = (
    ...
    def _run(self, **caps):
        ...
        with patch('coops.bronze.issues.load_json_data', return_value=self.REPOS), patch('coops.bronze.issues.save_json_data', side_effect=capture_save):

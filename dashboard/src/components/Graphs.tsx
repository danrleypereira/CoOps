...
import type { PieDatum, BasicDatum, CollaborationEdge, HeatmapDataPoint, HistogramDatum } from '../types';
 ...
 * Supports toggling between line and bar graph views.
 *
 * @param data - Array of histogram data points with date labels and counts
 * @param type - Label for the type of data being displayed
 */
export function Histogram({ data, type }: { data: BasicDatum[], type: string }) {
  ...
  useEffect(() => {
    ...
    // Line chart overlay
    const lineFn = d3.line<BasicDatum>()
    ...
    const node = container.append("g")
      ...
      .join("circle")
        .attr("r", (d: any) => {
          const degree = d.degree || 1;
          const logScale = Math.log(degree + 3);
          return Math.min(logScale * 3 + 3, 9);
        })
        .attr("fill", (d: any) => {
          const degree = d.degree || 0;
          if (degree < 3) return "#3498db";
          if (degree < 10) return "#e67e22";
          if (degree < 20) return "#e74c3c";
          return "#c0392b";
        })
        ...
        .on("mouseenter", (event: any, d: any) => {
          ...
          select(event.currentTarget as SVGCircleElement)
            ...
            .attr("r", (n: any) => {
              ...
              const logScale = Math.log(degree + 3);
              return Math.min(logScale * 3 + 3, 8) + 3;
            })
            .attr("stroke-width", 3);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="text-xs text-gray-400 px-2 py-1 bg-gray-900 text-center">
        Showing {graphData.nodes.length} collaborators with {graphData.links.length} connections
...
export function ActivityHeatmap({
  ...
  const svg = select(svgRef.current);
    ...
    if (graphWidth <= 0 || graphHeight <= 0) {
        console.warn("[Heatmap] Invalid dimensions for drawing.");
        return;
    }
  ...
  const maxCount = max(processedData, d => d.activity_count) || 1;
    ...
    cells.append("title")
          .text(d => `Day: ${dayLabels[d.day_of_week]}, Hour: ${d.hour}, Activities: ${d.activity_count}`);
  ...
  useEffect(() => {
    ...
    if (xMin && xMax && +xMin === +xMax) {
      if (timeRange === 'Last 24 hours') {
        xMax = new Date(xMax.getTime() + 60 * 60 * 1000);
      } else if (timeRange === 'Last 7 days' || timeRange === 'Last 30 days') {
        xMax = new Date(xMax.getTime() + 24 * 60 * 60 * 1000);
      } else {
    ...
    xAxisEl.selectAll('.tick line').style('stroke-opacity', 0);
    xAxisEl.selectAll('.tick text').attr('fill', '#777').style('text-anchor', 'middle');
    ...
    g.append('path')
      ...
      .attr('stroke', '#3b82f6')
      .attr('stroke-width', 2)
      .attr('d', lineFn as unknown as any);
...
export function CommitMetricsChart({ data, line_toggle }: { data: BasicDatum[] , line_toggle: boolean}) {
  ...
  useEffect(() => {
    ...
    // X Axis
    const tickIntervalX = Math.max(1, Math.floor(data.length / 15));
    const tickValuesX = data
      .map((d, i) => ({ v: d.date, i }))
      .filter((x) => x.i % tickIntervalX === 0)
      .map((x) => x.v);
    ...
    xAxisEl.selectAll('line').style('stroke', '#334155');
    xAxisEl.select('.domain').style('stroke', '#334155');
    ...
    const legendW = width - margin.left - margin.right;
    const itemSpacing = legendW / legendItems.length;

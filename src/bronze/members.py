...
Extracts raw member data from GitHub API
"""
...
def extract_members(client: GitHubAPIClient, config: OrganizationConfig, use_cache: bool = True) -> List[str]:
    """Extract organization members to bronze layer"""
    ...
    if not raw_members or len(raw_members) == 0:
        print("⚠️  Organization members API returned empty. This could be due to:")
        ...
        # Fallback: Get contributors from public repositories
        ...
        if repos_data and isinstance(repos_data, list):
            ...
            # Collect unique contributors across repositories
            contributors_set = set()
            contributor_details = {}
            
            for repo in repos_data[:5]:  # Check first 5 repos for broader coverage
    ...
    if not raw_members:
        print("⚠️  No members data available, but continuing with empty dataset")
        raw_members = []

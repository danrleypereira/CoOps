...
def check_no_orphaned_files(bronze: Path, rep: Report) -> None:
    """#216: every per-repository Bronze file belongs to the current listing.

    A repository that is renamed or recased leaves its old Bronze files on
    disk beside the new ones (`2025-1-NoFluxoUNB` and `2025-1-NoFluxoUnB`
    are one repository, id 957040204). The pipeline writes the current-case
    file; the old-case file is never written again and never removed. Silver
    and Gold consume Bronze per-repository files by glob, so both are read
    and the repository is counted twice in published data.

    No other check here can see it, structurally: they assert that records
    must not FALL, and the orphan ADDS records — counts go up, nothing
    vanishes, and the registry, generated by scanning the layers, lists the
    orphan as a legitimate file. A check answering "did we lose anything"
    cannot see "did we gain something that is not real". This one answers
    the second question, against `repositories_filtered.json`.

    The correspondence rule mirrors `coops.bronze.reconcile` deliberately
    rather than importing it — the verifier must not share its enumeration
    with the code under test. A file is orphaned when its repository is
    absent from the listing, or when it is an old-case spelling of a
    repository whose current-case file sits beside it (the rename leftover).
    An old-case file that is the ONLY copy for its repository is not an
    orphan: the run skipped or has not yet written that family, the file is
    the live data, and the reconciler keeps it — flagging it here would
    demand the deletion of an only copy, which is the record loss every
    other check in this file exists to prevent.

    Exit codes: an orphan is bad DATA — rc 1, the file named. A missing or
    unreadable listing is the INSTRUMENT — rc 2 — because which file is an
    orphan cannot be known without it, and a check that cannot see must not
    read as a pass.
    """
    listing = bronze / "repositories_filtered.json"
    if not listing.is_file():
        rep.add(Result("no-orphaned-bronze-files", "bronze", False,
                       f"{listing.name} is absent — which files are orphaned cannot be known",
                       control_fired=False))
        return
    try:
        ...
        rep.add(Result("no-orphaned-bronze-files", "bronze", False,
                       f"{listing.name} cannot be read ({exc})", control_fired=False))
        return
    if not isinstance(payload, list):
        rep.add(Result("no-orphaned-bronze-files", "bronze", False,
                       f"{listing.name} does not hold a list of repositories",
                       control_fired=False))
        return
    ...
    for name in sorted(names):
        current_case.setdefault(name.casefold(), name)

    orphans: list[str] = []
    checked = 0
    for family in BRONZE_FAMILIES:
        for path in bronze_family_files(bronze, family):
            checked += 1
            repo = path.stem[len(family) + 1:]
            if repo in names:
                continue
            current = current_case.get(repo.casefold())
            if current is None:
                orphans.append(f"{path.name} (repository absent from the listing)")
            elif (bronze / f"{family}_{current}.json").is_file():
                orphans.append(f"{path.name} (old case of {current})")
            # else: the only copy for its repository — see the docstring.

    if not orphans:
        rep.add(Result("no-orphaned-bronze-files", "bronze", True,
                       f"{checked} per-repository files checked against "
                       f"{len(names)} listed repositories, 0 orphaned"
                       + ("" if names else " — the listing holds no repositories")))
        return
    rep.add(
        Result(
            ...
            "bronze",
            False,
            f"{checked} per-repository files checked against {len(names)} listed "
            f"repositories, {len(orphans)} orphaned"
            + ("" if names else " — the listing holds no repositories")
            + f" (showing first 5 of {len(orphans)}): {', '.join(orphans[:5])}",
        )
    )
...
def run_controls(tmp: Path) -> list[Result]:
    ...
    # no-orphaned-bronze-files, plant: #216 itself — a recased repository
    # (NoFluxoUNB -> NoFluxoUnB, here X -> x) leaves the old-case file
    # beside the current one, both are globbed downstream, and the repo is
    # counted twice. The plant mirrors the real corpus's shape, listing and
    # both files, so what fires is the correspondence rule and not the
    # instrument branches.
    orc = tmp / "orphan-bronze"
    orc.mkdir(parents=True, exist_ok=True)
    (orc / "repositories_filtered.json").write_text(json.dumps([
        {"_metadata": {"extracted_at": "2026-09-25T00:00:00Z"}},
        {"name": "x", "full_name": "unb-mds/x", "id": 957040204},
    ]), encoding="utf-8")
    (orc / "commits_x.json").write_text("[]", encoding="utf-8")
    (orc / "commits_X.json").write_text("[]", encoding="utf-8")
    r = Report()
    check_no_orphaned_files(orc, r)
    res = r.results[0]
    fired = (not res.passed and r.exit_code == 1
             and "commits_X.json" in res.detail
             and "old case of x" in res.detail)
    out.append(Result("no-orphaned-bronze-files", "control", fired,
                      "rejects the recase leftover beside the current-case file, naming it"
                      if fired else "DID NOT FIRE"))

    # Arms differ: the same corpus with the orphan removed passes. A check
    # that fails on everything certifies nothing — this is the half that
    # proves the FAIL above was the orphan and not the corpus.
    (orc / "commits_X.json").unlink()
    r = Report()
    check_no_orphaned_files(orc, r)
    res = r.results[0]
    clean_ok = res.passed and r.exit_code == 0
    out.append(Result("no-orphaned-bronze-files-arms-differ", "control", clean_ok,
                      "the same corpus with the orphan removed passes" if clean_ok else "DID NOT FIRE"))
...
def main() -> int:
    ...
    if (data / "bronze").is_dir():
        ...
        check_no_orphaned_files(data / "bronze", rep)
        if args.reference:

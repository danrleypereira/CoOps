...
- **But invent the fixture when you are pinning a boundary the data does not happen to cross.** A real payload teaches you what the data looks like; it cannot defend a case reality never produced. Measured on the `fga-eps-mds` corpus, over the population the guard actually sees — `commit.author.name` and `commit.committer.name`, **520,700** values across 260,349 commits: 298 are entirely an email address and **zero** merely contain one. (Widening to every `name` key anywhere in Bronze gives 693,667 values, 562 addresses and 2 contains-only — but those 2 are `.png` filenames in file-tree entries, a population this scrub never touches. Quote the population the guard sees, not the larger one it does not.) So the rule that a name like `"Alice <alice@example.com>"` must be preserved while a bare address is blanked cannot be pinned by any real record, and a `fullmatch`→`search` mutation passes the whole suite. That boundary needs an invented fixture, and the fixture is not a weaker test for being invented — it is the only one that can fail. Say in a comment why it is synthetic, so the line above does not get it deleted.
